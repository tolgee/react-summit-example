...
FROM docker.io/library/node:24.4.0-alpine3.22 as frontend-build
WORKDIR /app
COPY package*.json ./
RUN npm install
COPY . .
...
FROM docker.io/library/node:24.4.0-alpine3.22 as server-build
WORKDIR /app
...
FROM docker.io/library/node:24.4.0-alpine3.22
WORKDIR /app
