...
FROM docker.io/library/node:26.5.1-alpine3.24 as frontend-build
WORKDIR /app
COPY package*.json ./
RUN npm install
COPY . .
...
FROM docker.io/library/node:26.5.1-alpine3.24 as server-build
WORKDIR /app
...
FROM docker.io/library/node:26.5.1-alpine3.24
WORKDIR /app
