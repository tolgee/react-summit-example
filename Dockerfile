...
FROM docker.io/library/node:25.8.1-alpine3.23 as frontend-build
WORKDIR /app
COPY package*.json ./
RUN npm install
COPY . .
...
FROM docker.io/library/node:25.8.1-alpine3.23 as server-build
WORKDIR /app
...
FROM docker.io/library/node:25.8.1-alpine3.23
WORKDIR /app
